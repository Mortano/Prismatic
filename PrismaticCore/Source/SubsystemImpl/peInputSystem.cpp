...
void peInputSystem::Init() {
  ...
  memset(_mouseStates, 0,
         static_cast<uint8_t>(MouseButton::NUM_MOUSEBUTTONS) *
             sizeof(KeyState));
  ...
  while (PeekMessage(&msg, NULL, 0, 0, PM_REMOVE)) {
    ...
    case WM_INPUT: // Handle remaining raw input
    {
      ...
      if (dwSize > 0) {
        ...
        UINT bytesWritten =
            GetRawInputData((HRAWINPUT)msg.lParam, RID_INPUT, buffer, &dwSize,
                            sizeof(RAWINPUTHEADER));
        PE_ASSERT(dwSize == bytesWritten);
        RAWINPUT *raw = (RAWINPUT *)buffer;
        if (raw->header.dwType == RIM_TYPEMOUSE) {
          _mouseDelta.x = raw->data.mouse.lLastX;
          _mouseDelta.y = raw->data.mouse.lLastY;
        }
      }
    } break;
    default:
...
glm::ivec2 peInputSystem::GetMouseDelta() const { return _mouseDelta; }
